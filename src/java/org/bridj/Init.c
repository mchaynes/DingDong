...
JNIEXPORT void JNICALL Java_org_bridj_v0_16_12_Platform_init(JNIEnv *env, jclass clazz);

const char* getBridJLibPath()
{
	const char* libPath;
	Dl_info info;
	dladdr(Java_org_bridj_v0_16_12_Platform_init, &info);
...
JNIEXPORT void JNICALL Java_org_bridj_v0_16_12_Platform_init(JNIEnv *env, jclass clazz)
{
	...
	jclass signatureHelperClass = (*env)->FindClass(env, "org/bridj/v0_6_2/util/JNIUtils");
	...
	for (iSym = 0; iSym < nSyms; iSym++) {
		...
		if (!strcmp(*symbolName == '_' ? symbolName + 1 : symbolName, "Java_org_bridj_v0_16_12_Platform_init"))
			continue;
